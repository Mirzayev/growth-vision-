...
export default function Banner() {
  ...
  // Avval variantlarni tashqarida e’lon qilamiz
const container = {
  hidden: { opacity: 0 },
  visible: {
    opacity: 1,
    transition: {
      staggerChildren: 0.25, // 🔥 bolalar ketma-ket chiqadi
    },
  },
};
      ...
      <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-16 items-center max-w-7xl w-full">
        ...
        <div className="grid grid-cols-2 gap-6">
          {features.map((f, i) => (
            <motion.div
              key={i}
              initial={{ opacity: 0, y: 60 }} // 🔥 boshlanish pastroqda
              animate={{ opacity: 1, y: 0 }}
              transition={{
                delay: i * 0.25, // 🔥 har kartochka ketma-ket chiqadi
                duration: 0.9, // 🔥 biroz sekinroq
                ease: "easeOut", // 🔥 chiqishi mayinroq
              }}
